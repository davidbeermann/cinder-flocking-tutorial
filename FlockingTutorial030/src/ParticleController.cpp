...
void ParticleController::applyFlocking(float zoneRadius, float threshold) {
    ...
    for (PIt p1 = mParticles.begin(); p1 != mParticles.end(); ++p1) {
        ...
        for (++p2; p2 != mParticles.end(); ++p2) {
            ...
            if (distSqrd <= zoneRadiusSquared) { // Neighbor is in the zone
                ...
                if (percent < threshold) { // SEPARATION
                    ...
                    float thresholdDelta = 1.0f - threshold;
                    float adjustedPercent = (percent - threshold) / thresholdDelta;
                    float F = (1.0f - (cos(adjustedPercent * twoPI) * -0.5f + 0.5f)) * 0.04f;
