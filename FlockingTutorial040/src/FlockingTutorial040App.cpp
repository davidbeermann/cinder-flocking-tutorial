...
class FlockingTutorial040App : public App {
public:
    void setup() override;
...
void FlockingTutorial040App::setup() {
    // test force algorithm output
//    float twoPI = (float) M_PI * 2.0f;
//    for (float i = 0.0f; i < 1.05f; i += 0.05f) {
//        float theta = cos(i * twoPI);
//        float F = 1.0f - (theta * 0.5f + 0.5f);
//        console() << i << " -> theta: " << theta << " - force: " << F << endl;
//    }
...
void FlockingTutorial040App::update() {
...
void FlockingTutorial040App::draw() {
